...
| `io/importSvg.ts` | SVG file → `RigDoc`. Unwraps Inkscape layers, then RECURSIVE: every `<g>` at any depth becomes a part (exact SVG structure; label = inkscape:label else id; kind 'art' iff it has direct paths), parented per the nesting; each part's baked `transform` is the FULL composed ancestor chain (doc-space invariant — render-time parenting composes pose only); ellipse/circle/rect → path data; pivots per part from the *composed matrix's fixed point* or, when none is recovered, a `PivotHint` (an `inkscape:transform-center-x/y` `centerOffset`, else a plain `bboxCenter` placeholder marker — recorded EXPLICITLY, never inferred from `pivot==(0,0)`, since a composed ancestor translate can leave the placeholder off-origin); hints resolve once geometry is measurable — live by the canvas in-app, pure-doc by `partHierarchy.ts`'s `seedImportedPivots` (subtree geometry bbox center) for a headless import, wired into `importSvgHeadless` so CLI/MCP/golden all seed; U4: every imported part gets an EXPLICIT `childOrder` recording the group's true child interleaving in document order (one slot appended per child during the walk) |
...
| `geometry/warp.ts` | Warp Transition compiler: durable group/path correspondence, exact-name auto-match, line/arc→cubic normalization, deterministic topology equalization, seam/direction repair, transformed endpoint alignment, interpolation and staleness checks. The live renderer morphs before LBS skinning; `io/riv/warp.ts` maps the result to native vertex/handle/paint keys. |
| `panels/warpWorkspace.ts` | Focused responsive Warp Setup dock: source/reference roles, complete confirmed/unmatched correspondence list, live scrub, manual pair/unpair, direction/seam repair, safe rebuild, and timeline key authoring. Opened from the compact canvas-tools Warp action for a two-object selection. |
...
| `geometry/pathBounds.ts` | Pure path-data bounding boxes (no DOM): reuses `parsePath`/`pathToCubics`, solves cubic interior extrema analytically, maps control points through an affine matrix BEFORE extrema (exact). Powers `memberGeometryPivot` — headless `groupParts` with no explicit pivot defaults to the members' geometry bbox center (partHierarchy.ts; the in-app Ctrl+G flow already measured the live DOM) |
...
- **Unified child ordering (the U-program, COMPLETE as of U4 2026-07-14)** layers on
  ...
  paint via `core/paintOrder.ts`'s `flattenPaintOrder` run-splitting (contiguous own
  ...
  skin/pin-anchor emission memoized once per part; keyed-z now uses the same sibling-
  scoped flatten at the union of every z event, with per-drawable fixed-rank anchors so
  interleaved/subtree ordering stays exact in `.riv`). U4: the importer records TRUE SVG document order into slots
...
- **Pivots are recovered from the composed transform matrix, not the transform-list
  ...
  rotation and solves for its fixed point either way. When none is recovered the importer
  records an EXPLICIT `PivotHint` (never inferred later from `pivot==(0,0)` — a composed
  ancestor translate can leave the placeholder off-origin): `inkscape:transform-center-x/y`
  becomes a `centerOffset` hint (bbox-center offset, +y flipped from Inkscape's legacy
  axis), else a plain `bboxCenter` hint. Hints resolve once geometry is measurable — live
  by the canvas in-app (`view/canvas.ts`), pure-doc by `seedImportedPivots` (subtree
  geometry center) for a headless import so the CLI/MCP don't leave far-orbit placeholder
  pivots. Saved docs are never re-seeded on load (normalizeDoc leaves pivots alone).
