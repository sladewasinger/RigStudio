 ...
 * object sections), `ai.ts` (the Claude assistant panel, mounted at the bottom of the
 * inspector), and `canvasTools.ts` (the tool switcher + snap toggle + flip/group/
 * ungroup/bind actions shown above the canvas). `smPanel.ts` (the state-machine editor)
 * lives alongside these but is imported directly by its consumers (main, timeline), not
 * re-exported here.
 *
...
export { buildCanvasTools, flipAction, groupAction, ungroupAction, bindAction } from './canvasTools';
