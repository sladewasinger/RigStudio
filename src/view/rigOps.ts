 ...
 * `rigOpsNodeBinding.ts` (per-node skin weight override refinement), and
 * `rigOpsPlacement.ts` (pen-tool bone-chain placement lifecycle + Bones 2.0 auto-bind
 * targeting). These mutate the doc (rest pose, geometry, skin) and repaint; the caller
 * checkpoints history where appropriate. This file re-exports exactly the surface
 * `view/index.ts` and `view/interactions.ts` consume; nothing outside those two reaches
 * past it into a submodule, and no submodule imports this facade back.
 */
